...
namespace KooliProjekt.Application.UnitTests.Features.Invoices
{
    public class DeleteInvoiceCommandHandlerTests : TestBase
    {
        ...
        public async Task Delete_should_return_when_request_id_is_null_or_negative(int id)
        {
            ...
            var handler = new DeleteInvoiceCommandHandler(faultyDbContext);
        ...
        public async Task Delete_should_remove_existing_invoice()
        {
            ...
            var command = new DeleteInvoiceCommand { InvoiceId = invoice.InvoiceId };
            var handler = new DeleteInvoiceCommandHandler(DbContext);

            // Act
            var result = await handler.Handle(command, CancellationToken.None);
            var deletedInvoice = await DbContext.Invoices.FindAsync(command.InvoiceId);
        ...
        [Fact]
        public async Task Delete_should_not_fail_when_invoice_does_not_exist()
        {
            // Arrange
            var doctor = new Doctor
            {
            ...
            var command = new DeleteInvoiceCommand { InvoiceId = 999 };
            var handler = new DeleteInvoiceCommandHandler(DbContext);

            // Act
            var result = await handler.Handle(command, CancellationToken.None);
            var existingInvoice = await DbContext.Invoices.FindAsync(command.InvoiceId);

            // Assert
            Assert.NotNull(result);
            Assert.False(result.HasErrors);
            Assert.Null(existingInvoice);
        }
    }
}
