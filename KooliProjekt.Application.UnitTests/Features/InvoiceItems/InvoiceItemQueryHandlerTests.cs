...
{
    public class InvoiceItemQueryHandlerTests : TestBase
    {
        [Fact]
        public async Task List_should_return_paged_results()
        {
            ...
            var query = new InvoiceItemsQuery { Page = 1, PageSize = 10 };
            var handler = new InvoiceItemQueryHandler(DbContext);
        ...
        public async Task List_should_throw_ArgumentException_when_page_is_zero_or_less(int page)
        {
            ...
            var query = new InvoiceItemsQuery { Page = page, PageSize = 10 };
            var handler = new InvoiceItemQueryHandler(dbContext);
        ...
        public async Task List_should_throw_ArgumentException_when_pageSize_is_zero_or_less(int pageSize)
        {
            // Arrange
            var dbContext = GetFaultyDbContext();
            var query = new InvoiceItemsQuery { Page = 1, PageSize = pageSize };
            var handler = new InvoiceItemQueryHandler(dbContext);
